            ...
            <div className="space-y-4">
              {faqData.map((faq) => {
                ...
                return (
                  <div
                    key={faq.id}
                    className="bg-gray-900 rounded-xl border border-gray-700 hover:border-yellow-500 transition-all duration-300 overflow-hidden"
                  >
                    <button
                      onClick={() => toggleAccordion(faq.id)}
                      className="w-full px-6 py-6 text-left focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-inset"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-start gap-4">
                          <div className={`flex-shrink-0 w-10 h-10 bg-${colorClass}-500/20 rounded-lg flex items-center justify-center`}>
                            <CategoryIcon className={`w-5 h-5 text-${colorClass}-400`} />
                          </div>
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              <span className={`px-3 py-1 bg-${colorClass}-500/20 text-${colorClass}-400 text-xs font-semibold rounded-full`}>
                                {faq.category}
                              </span>
                            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {categories.slice(0, 6).map((category, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="bg-gray-800 rounded-xl p-6 border border-gray-700 hover:border-yellow-500 transition-all duration-300 text-center"
                  >
                    <div className={`w-16 h-16 bg-${category.color}-500/20 rounded-full flex items-center justify-center mx-auto mb-4`}>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl p-12 border border-gray-700">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <a
                  href="/contact"
                  className="px-8 py-4 bg-gradient-to-r from-yellow-500 to-yellow-600 text-black font-semibold rounded-lg hover:from-yellow-600 hover:to-yellow-700 transform hover:scale-105 transition-all duration-200"
                >
                  Contact Support
                </a>
                <a
                  href="https://platform.skillgame.pro/register"
                  className="px-8 py-4 border border-yellow-500 text-yellow-500 font-semibold rounded-lg hover:bg-yellow-500 hover:text-black transition-all duration-200"
                >
                  Start Playing Now
                </a>
              </div>
